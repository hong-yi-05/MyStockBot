...
for ticker, name in stock_dict.items():
    ...
    try:
        ...
        # 🌟 核心修復：清掉空白行，確保收盤價不會變 None
        hist = hist.dropna()
        if len(hist) < 100: continue
        
        close_px, volume = hist['Close'].squeeze(), hist['Volume'].squeeze()
        open_px, high_px, low_px = hist['Open'].squeeze(), hist['Low'].squeeze()
        ...
        if score >= 3:
            ...
            results.append({
                ...
                "股票代號": code, "股票名稱": name, "最新收盤價": round(float(curr_c), 2),
